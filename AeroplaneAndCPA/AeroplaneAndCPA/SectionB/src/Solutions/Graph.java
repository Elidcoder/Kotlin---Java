...
public class Graph implements GraphInterface {
	...
	private QueueInterface<GraphNode> topologicalSort( ) {
	  ...
	  QueueInterface<GraphNode> tempQueue = new Queue<>();
	  ...
	  while (!tempQueue.isEmpty()) {
	    GraphNode graphNode = tempQueue.dequeue();
	    resultQueue.enqueue(graphNode);

      for (GraphEdge edge : graphNode.getOutgoingEdges()) {
        GraphNode current = edge.getEndPoint();
	...
	private void computeEarliestCompletionTime( ) {
      ...
      GraphNode current = sortedNodes.dequeue();
      if (current.getIncomingEdges().size() == 1) {
        GraphEdge currentEdge = current.getIncomingEdges().get(1);
        current.setEarliestCompletionTime(currentEdge.getTaskDuration()
            + currentEdge.getStartPoint().EarliestCompletionTime());
      } else {
        int max = 0;
        for (GraphEdge currentEdge : current.getIncomingEdges()) {
